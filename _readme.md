...
The `#<name>`'s will be expanded (once) using the value of the option they
refer to for each codeblock that is about to be processed. The values for
`#sha` and `#oid` are provided by stitch, where `oid` is the codeblock's
identifier or is generated as `anon<nr>` with `nr` being the nth codeblock
seen.
...
[`arg`] attribute if needed and possibly setting its [`dir`] and or [`inc`]
attribute.
