...
set of options found for this codeblock. See [Options](#options) below for the
resolution order.

Using `stitch=` or `stitch=""` is the same as included the `.stitch` class.
...
can be processed further by listing a filter in the form of `mod.func`.  If
defined, the lua-module `mod` will be required and its `func` called with the
data read.  The `mod.func` is passed the data, which could be image data, plain
ascii text or a pandoc doc (if *read* was used).
