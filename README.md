...
will be processed due to the directive: \`out!markdown@src.suture.header\` which
says:
...
This report could’ve been dynamically generated by some `cli` command to
e.g. stdout. Instead it is echo’d to the terminal by this codeblock and
that output will be processed due to the directive:
`out!markdown@src.suture.header` which says:
