...
- an `stitch=name` attribute, explicitly linking it to a
...
- `.stitch` class (which uses `doc.meta.stitch.defaults` or hard-coded
  defaults)
...
2.  save codeblock’s `text` to [`cbx`](#cbx)-file & mark it as
    executable (always)
3.  check if anything has changed, see [`oid`](#oid) (1+ of the other
    artifacts exist)
4.  conditionally run [`cmd`](#cmd) or loadfile the [`cbx`](#cbx)-file &
...
- log levels, global, per tool or codeblock, to show some details
  ([`log`](#log))
...
- include an artifact after converting it using a [pandoc
...
- run the [`cbx`](#cbx) or other artifact through an external filter
  ...
  - possible stitch itself to handle codeblocks in an acquired markdown
    doc
...
cbx-file  
the [`cbx`](#cbx)-file where a codeblock’s content is stored and marked
executable
...
The `#<name>`’s will be expanded (once) using the value of the option
they refer to for each codeblock that is about to be processed. The
values for `#sha` and `#oid` are provided by stitch, where `oid` is the
codeblock’s identifier or is generated as `anon<nr>` with `nr` being the
nth codeblock seen.

The defaults are basically setup to:

- run the [`cbx`](#cbx)-file as a system command
...
specifying an [`arg`](#arg) attribute if needed and possibly setting its
[`dir`](#dir) and or [`inc`](#inc) attribute.
...
`os.execute(cmd)`. The hard-coded default is `arg=""`, which won’t show
...
their hard-coded values. The `sha` and `oid` are provided by stitch and
...
needed, simply ignore the option and configure a more appropiate
...
command to run. That is not a requirement though. Perhaps unusual, but
one can do:
...
to process using the stitch filter. In that scenario the
[`inc`](#inc)-option is setup to convert the data to a pandoc AST and
then filter it using stitch.

That automatic linking can be prevented by setting `cls=no` in either
the relevant stitch-section in `doc.meta` or in a codeblock’s
attributes.
...
directory where pandoc was started. Override the hard-coded `.stitch`
...
detect changes, stitch fingerprints a codeblock using:
...
Although not particularly useful when converting a document directly,
the actual purpose is to use the `hdr`-option in a codeblock’s
attributes that is pulling in another markdown document. This provides a
mechanism to slide the headers of the included document up or down (when
filtering the document with stitch).

Whenever stitch runs data through an additional filter and data is a
pandoc document, the current documents meta data is added to the AST,
and:
...
Note: the `hdr`-value is taken from the codeblock’s attribute section
only and not resolved, whereas the `log`-option will be resolved to a
value. So to silence logging during recursion (or crank it up instead),
be sure to set the codeblock’s `log`-option to its desired value. As an
example:
...
See [Nested doc](#nested-doc) for an example.
...
its meta data is augmented with the main document’s meta data without
overwriting. Also, `doc.meta.stitch.stitch.hdr` is set to (forcibly) to
the codeblock’s [`hdr`](#hdr)-option value (if present).

Note however, that data need not be a pandoc AST and `mod` could be any
lua module that simply accepts the data as acquired by reading the
`what`-file.

If no module was found an error is logged and the `inc`-directive is
...
For reference in the following examples, here is this document’s
doc.meta:
...
stitch itself. The codeblock has `hdr=1`, which means the all header
levels of the markdown being included will be incremented by one.

````` lua
```` {#nested .lua .stitch inc="cbx:fcb out!markdown@stitch" log="silent" cls="yes" hdr="1"}
#! /usr/bin/env lua
